...
const adapters: MetadataAdapter[] = [imageAdapter, pdfAdapter, ooxmlAdapter, mp4Adapter, mp3Adapter];
...
export async function parseFile(file: File, allowedTypes?: DetectedFileType[]): Promise<ParsedMetadata> {
  ...
  if (!adapter) throw new MetadataError('UNSUPPORTED_FILE_TYPE', 'Supported formats are JPEG, PNG, WebP, PDF, DOCX, PPTX, XLSX, MP4, and MP3.');
