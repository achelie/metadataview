...
function SafeJsonDetails({ title, note, value, className = '', locale = 'en' }: { title: string; note: string; value: unknown; className?: string; locale?: Locale }) {
  ...
  return <details className={`c2pa-json-details ${className}`} onToggle={(event) => setOpen(event.currentTarget.open)}>
    <summary className="disclosure-summary"><span className="disclosure-label">{title}<small>{note}</small></span><span className="disclosure-controls"><b>{open ? (locale === 'zh-CN' ? '关闭' : locale === 'de' ? 'Schließen' : locale === 'fr' ? 'Fermer' : 'Close') : (locale === 'zh-CN' ? '打开' : locale === 'de' ? 'Öffnen' : locale === 'fr' ? 'Ouvrir' : 'Open')}</b><DisclosureChevron /></span></summary>
