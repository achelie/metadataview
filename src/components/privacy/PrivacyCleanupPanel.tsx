...
import { localizePrivacyCleanupCheck, localizePrivacyCleanupStage, localizePrivacyDiagnostic, privacyCleanupText } from '../../i18n/privacy-cleanup';
...
export function PrivacyCleanupPanel({ report, metadata, sourceFile, mode, pending, baselinePending = false, variant = 'checker', stage, error, result, onMode, onClean, onDownload, onReceipt, onReset, locale = 'en' }: {
  ...
  const outputFile = useMemo(() => result && result.verificationStatus !== 'failed' ? new File([result.blob], result.fileName, { type: result.mime }) : null, [result]);
  const continueLink = (href: string, label: string) => sourceFile ? <ToolFileLink file={sourceFile} href={href} className="inline-link">{label}</ToolFileLink> : <a href={href}>{label}</a>;
  const modesHeading = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (!result) return;
  ...
  const remainingCount = result?.diff?.remainingRiskIds.length ?? 0;
  const addedCount = result?.diff?.addedRiskIds.length ?? 0;
  const verificationComplete = result?.verificationStatus === 'verified' && Boolean(result.diff);
  const verifiedClear = verificationComplete && remainingCount + addedCount === 0;
  const verdict = integrityFailed ? 'verificationFailed' : !verificationComplete ? 'verificationIncomplete' : verifiedClear ? 'verifiedClear' : 'verifiedResidual';
  const reason = integrityFailed ? 'failedLimit' : !verificationComplete ? 'incompleteLimit' : verifiedClear ? 'clearLimit' : 'residualLimit';
  const diagnostic = error ? localizePrivacyDiagnostic(error, locale, 'error') : null;
  ...
  const changeMethod = () => {
    onReset?.();
    window.requestAnimationFrame(() => modesHeading.current?.querySelector<HTMLButtonElement>('[aria-checked="true"]')?.focus());
  };

  return <section className="privacy-cleanup" aria-labelledby={result ? 'privacy-cleanup-result-heading' : 'privacy-cleanup-heading'} aria-busy={pending}>
    {!result && <>
      ...
      <div ref={modesHeading} className="privacy-cleanup-modes" role="radiogroup" aria-label={t('Cleanup method', '清理方式', 'Bereinigungsmethode')}>
    ...
    {!result && <div className="privacy-cleanup-run"><div><b>{baselinePending ? t('Finishing the full source scan before cleanup.', '清理前先完成源文件全量扫描。', 'Der vollständige Quellscan wird vor der Bereinigung beendet.') : report.risks.length ? privacyCleanupText(locale, 'cleanupCandidates', { count: report.risks.length }) : t('A zero score is not a safety guarantee.', '0 分不是安全保证。', 'Ein Wert von null ist keine Sicherheitsgarantie.')}</b><span>{pending ? privacyCleanupText(locale, 'workingStage', { stage: localizePrivacyCleanupStage(stage, locale) }) : baselinePending ? t('Using the same complete scan before and after prevents misleading score changes.', '前后使用同一套完整扫描，避免分数变化造成误导。', 'Derselbe Vollscan davor und danach verhindert irreführende Wertänderungen.') : t('The clean copy gets the same one-pass full scan before it is marked verified.', '清理副本也会跑同一套完整扫描，通过后才标记为已验证。', 'Auch die bereinigte Kopie erhält denselben Vollscan, bevor sie als verifiziert gilt.')}</span></div><button className="button button-primary" type="button" disabled={pending || baselinePending || (privacyFirstDisabled && mode === 'privacy-first')} onClick={onClean}><Icon icon={shieldIcon} width="17" />{pending ? t('Cleaning and verifying…', '正在清理并验证…', 'Bereinigung und Prüfung…') : baselinePending ? t('Preparing full baseline…', '正在准备完整基线…', 'Vollständige Ausgangsbasis wird erstellt…') : t('Create and verify clean copy', '创建并验证清理副本', 'Bereinigte Kopie erstellen und prüfen')}</button></div>}
    {diagnostic && <div className="privacy-cleanup-error" role="alert"><p>{diagnostic.message}</p>{diagnostic.original && originalDetails(diagnostic.original)}</div>}
    {result && <div className={`privacy-cleanup-result status-${result.verificationStatus}`}>
      <div className="privacy-cleanup-verdict">
        <Icon icon={fileIcon} width="28" aria-hidden="true" />
        <div>
          <h3 id="privacy-cleanup-result-heading" ref={resultHeading} tabIndex={-1}>{privacyCleanupText(locale, verdict)}</h3>
          <p className="privacy-cleanup-result-note">{privacyCleanupText(locale, reason)}</p>
          <p>{result.fileName} · {result.mode === 'privacy-first' ? t('privacy-first', '隐私优先', 'Datenschutz zuerst') : t('preserve-encoding', '保留编码', 'Codierung erhalten')}</p>
          <p className="privacy-cleanup-risk-counts">
            <strong>{result.diff ? privacyCleanupText(locale, verificationComplete ? 'remainingRisks' : 'observedRemainingRisks', { count: remainingCount }) : privacyCleanupText(locale, 'unknownRemaining')}</strong>
            {result.diff && <strong>{privacyCleanupText(locale, verificationComplete ? 'addedRisks' : 'observedAddedRisks', { count: addedCount })}</strong>}
          </p>
        </div>
      </div>
      <ResultActionBar className="privacy-cleanup-result-actions" label={`${privacyCleanupText(locale, 'resultActions')} · ${privacyCleanupText(locale, verdict)}`}>
        <button className="button button-primary" type="button" disabled={integrityFailed} onClick={onDownload}>
          <Icon icon={downloadIcon} width="16" aria-hidden="true" />{integrityFailed ? t('Download blocked: invalid output', '输出无效，已阻止下载', 'Download blockiert: ungültige Ausgabe') : verifiedClear ? t('Download clean copy', '下载清理副本', 'Bereinigte Kopie herunterladen') : privacyCleanupText(locale, 'downloadProcessed')}
        </button>
        <details className="result-export-menu">
          <summary className="disclosure-summary"><span className="disclosure-label">{privacyCleanupText(locale, 'moreActions')}</span><DisclosureChevron /></summary>
          <div className="result-export-options">
            <button className="button button-secondary" type="button" onClick={onReceipt}><Icon icon={downloadIcon} width="16" aria-hidden="true" />{t('Download cleanup receipt', '下载清理收据（英文）', 'Bereinigungsbeleg herunterladen (Englisch)')}</button>
            {onReset && <button className="button button-ghost" type="button" onClick={changeMethod}>{privacyCleanupText(locale, 'changeMethod')}</button>}
            <button className="button button-ghost" type="button" disabled={pending || baselinePending} onClick={onClean}>{t('Run cleanup again', '再清理一次', 'Bereinigung wiederholen')}</button>
          </div>
        </details>
        {!verifiedClear && <span className="result-action-feedback">{integrityFailed ? privacyCleanupText(locale, 'verificationFailed') : !verificationComplete ? privacyCleanupText(locale, 'verificationIncomplete') : privacyCleanupText(locale, 'risksRemain', { count: remainingCount + addedCount })}</span>}
      </ResultActionBar>
      {outputFile && <ToolNextSteps output><ToolFileLink file={outputFile} href={localizePath('/image-metadata-viewer/', locale)}>{toolHandoffText(locale, 'metadata')}</ToolFileLink></ToolNextSteps>}
      <details className="privacy-cleanup-details" open={integrityFailed || undefined}>
        <summary className="disclosure-summary"><span className="disclosure-label">{privacyCleanupText(locale, 'verificationDetails')}</span><DisclosureChevron /></summary>
        <dl><div><dt>{privacyCleanupText(locale, 'riskScore')}</dt><dd>{result.diff ? `${result.diff.scoreBefore} → ${result.diff.scoreAfter}` : t('Not verified', '未验证', 'Nicht verifiziert')}</dd></div><div><dt>{t('Sensitive fields', '敏感字段', 'Sensible Felder')}</dt><dd>{result.diff ? `${result.diff.fieldsBefore} → ${result.diff.fieldsAfter}` : t('Unknown', '未知', 'Unbekannt')}</dd></div><div><dt>{t('File size', '文件大小', 'Dateigröße')}</dt><dd>{formatBytes(result.beforeSize)} → {formatBytes(result.afterSize)}</dd></div></dl>
        ...
        <div className="privacy-cleanup-checks">{result.outputChecks.map((check) => {
          const copy = localizePrivacyCleanupCheck(check, locale);
          return <div className={`privacy-cleanup-check is-${check.status}`} key={check.id}><b>{copy.label}</b><span>{copy.message}</span>{copy.original && originalDetails(copy.original)}</div>;
        })}</div>
        {result.warnings.length > 0 && <ul>{result.warnings.map((warning) => {
          const copy = localizePrivacyDiagnostic(warning, locale);
          return <li key={warning}><span>{copy.message}</span>{copy.original && originalDetails(copy.original)}</li>;
        })}</ul>}
      </details>
    </div>}
    <footer>{variant === 'remover' ? (zh ? <>分数只覆盖内嵌元数据，不包括输出文件名或画面像素。{continueLink(localizePath('/image-privacy-checker/', locale), "打开完整隐私检查器")}。</> : de ? <>Der Wert deckt eingebettete Metadaten ab, nicht Ausgabedateiname oder sichtbare Pixel. {continueLink(localizePath('/image-privacy-checker/', locale), "Vollständigen Datenschutz-Check öffnen")}.</> : fr ? <>Le score couvre les métadonnées intégrées, pas le nom de sortie ni les pixels visibles. {continueLink(localizePath('/image-privacy-checker/', locale), "Ouvrir le contrôle de confidentialité complet")}.</> : <>The score covers embedded metadata, not the output filename or visible pixels. {continueLink("/image-privacy-checker/", "Open the full Privacy Checker")}.</>) : (zh ? <>只想用一个更简单的单功能流程？{continueLink(localizePath('/image-metadata-remover/', locale), "可以用图片元数据清理器")}。这个检查器会保留更详细的前后证据。</> : de ? <>Brauchst du nur einen einfachen Ablauf? {continueLink(localizePath('/image-metadata-remover/', locale), "Nutze den Bild-Metadaten-Entferner")}. Dieser Check behält hier die ausführlichen Vorher-/Nachher-Nachweise.</> : fr ? <>Besoin d’un parcours plus simple ? {continueLink(localizePath('/image-metadata-remover/', locale), "Utilisez le suppresseur de métadonnées d’image")}. Ce contrôle garde ici les preuves détaillées avant/après.</> : <>Need a simpler one-purpose flow? {continueLink("/image-metadata-remover/", "Image Metadata Remover is available")}. This checker keeps the detailed before/after evidence here.</>)}</footer>
  </section>;
}
