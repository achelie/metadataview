...
export function PrivacyCleanupPanel({ report, metadata, mode, pending, baselinePending = false, variant = 'checker', stage, error, result, onMode, onClean, onDownload, onReceipt }: {
  ...
  return <section className="privacy-cleanup" aria-labelledby="privacy-cleanup-heading" aria-busy={pending}>
    ...
    {!result && <div className="privacy-cleanup-run"><div><b>{baselinePending ? 'Finishing the full source scan before cleanup.' : report.risks.length ? `${report.risks.length} risks are cleanup candidates.` : 'A zero score is not a safety guarantee.'}</b><span>{pending ? `Working locally: ${stage ?? 'preparing verification'}…` : baselinePending ? 'Using the same complete scan before and after prevents misleading score changes.' : 'The clean copy gets the same one-pass full scan before it is marked verified.'}</span></div><button className="button button-primary" type="button" disabled={pending || baselinePending || (privacyFirstDisabled && mode === 'privacy-first')} onClick={onClean}><Icon icon={shieldIcon} width="17" />{pending ? 'Cleaning and verifying…' : baselinePending ? 'Preparing full baseline…' : 'Create and verify clean copy'}</button></div>}
