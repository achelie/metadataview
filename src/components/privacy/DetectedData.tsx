...
export function DetectedData({ report }: { report: PrivacyReport }) {
  ...
  return <section className="privacy-detected-data"><div><span className="section-index">DETECTED METADATA</span><h2>The evidence used here</h2><p>This is the privacy-relevant subset, with masked previews and exact source paths.</p><a className="inline-link" href="/image-metadata-viewer">View all image metadata →</a><small>You will need to select the image again. The file is never carried between pages.</small></div><div className="privacy-data-ledger">{visible.length ? visible.map((field,index)=><div key={`${field.path}-${index}`}><span>Metadata field</span><strong>{field.label}</strong><code>{field.displayValue}</code><small>{field.groupPath ?? field.origin}<br />{field.path}</small><CopyButton value={field.displayValue} label="Copy preview" /></div>) : <p>No privacy-relevant fields were added to the report.</p>}{fields.length > 12 && <button className="button button-secondary" type="button" onClick={()=>setExpanded((value)=>!value)}>{expanded?'Show fewer fields':`Show all ${fields.length} fields`}</button>}</div></section>;
}
