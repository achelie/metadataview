...
export default function MetadataRemover() {
  ...
  const runDeepScan = async (file: File, parsed: NormalizedImageMetadata, previous: PrivacyReport, mode: MetadataInspectionMode, current: number) => {
    setDeepPending(true); setDeepMode(mode); setDeepStage('loading'); setCleanupResult(null); revokeDownload();
    setStatus(`${mode === 'embedded' ? 'Embedded' : 'Standard'} baseline scan running locally`);
    try {
      ...
      setStatus(`${mode === 'embedded' ? 'Embedded' : 'Standard'} baseline ready · ${inspection.report.risks.length} supported risks`);
    } catch (caught) {
      if (request.current !== current) return;
      ...
      setStatus(`${mode === 'embedded' ? 'Embedded' : 'Standard'} scan stopped · Quick cleanup remains available with incomplete verification`);
    } finally {
  ...
  const inspect = async (files: FileList | File[]) => {
    ...
    setQuickBusy(true); setStatus('Building the Quick privacy baseline');
    try {
      ...
      setStatus('Quick baseline ready · starting ExifTool Standard scan');
      void runDeepScan(file, quick.metadata, quick.report, 'standard', current);
    } catch (caught) {
  ...
  return <section className="workbench privacy-checker metadata-remover" aria-busy={quickBusy || deepPending || cleanupPending}>
    ...
    {!selection && <div className={`privacy-dropzone remover-dropzone ${dragging ? 'is-dragging' : ''}`} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); void inspect(event.dataTransfer.files); }}>
      <span className="privacy-drop-icon" aria-hidden="true"><Icon icon={uploadIcon} width="34" /></span><div className="privacy-drop-copy"><span className="eyebrow">Strip it, then prove it</span><strong>Drop an image here</strong><p id="remover-drop-help">JPEG, PNG, or WebP · up to 50 MB</p><button ref={chooseButton} className="button button-primary" type="button" onClick={() => { if (picker.current) { picker.current.value = ''; picker.current.click(); } }} aria-describedby="remover-drop-help">Choose an image</button></div><p className="privacy-check-scope">The clean copy is scored with the same rules as Privacy Checker before download.</p><small>Quick results appear first. ExifTool loads only after you choose an image.</small>
    </div>}
    ...
    {report && metadata && source && <div className="privacy-result-shell remover-result-shell">
      ...
      <PrivacyScanStatus report={report} pending={deepPending} stage={deepStage} mode={deepMode} onCancel={() => exifClient.current?.cancel()} onRetry={() => void runDeepScan(source, metadata, report, 'standard', request.current)} onEmbedded={() => void runDeepScan(source, metadata, report, 'embedded', request.current)} />
