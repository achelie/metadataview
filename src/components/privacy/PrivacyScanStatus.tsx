...
import { localizePrivacyCleanupStage, localizePrivacyDiagnostic, privacyCleanupText } from '../../i18n/privacy-cleanup';

export function PrivacyScanStatus({ report, pending, stage, onCancel, onRetry, locale = 'en' }: {
  ...
  const diagnostic = failed && exif?.message ? localizePrivacyDiagnostic(exif.message, locale, 'error') : null;
  ...
  return <section className={`privacy-engine-rail is-full-scan${failed ? ' is-failed' : complete ? ' is-complete' : ''}`} aria-live="polite" aria-busy={pending}>
    ...
    <div className="privacy-engine-copy"><span className="section-index">{t('ONE-PASS FULL SCAN', '单次完整扫描', 'VOLLScan IN EINEM DURCHGANG', 'ANALYSE COMPLÈTE EN UN PASSAGE')}</span><h2>{pending ? t('Scanning every metadata field…', '正在扫描所有元数据字段…', 'Alle Metadatenfelder werden gescannt…', 'Analyse de tous les champs…') : complete ? t('Full scan complete', '完整扫描完成', 'Vollscan abgeschlossen', 'Analyse complète terminée') : t('Full scan incomplete', '完整扫描未完成', 'Vollscan unvollständig', 'Analyse complète inachevée')}</h2><p>{pending ? t('ExifTool is checking standard tags, embedded previews, and nested image records locally. The current result may still change.', 'ExifTool 正在本地检查标准标签、内嵌预览和嵌套图片记录，当前结果仍可能变化。', 'ExifTool prüft Standard-Tags, eingebettete Vorschauen und verschachtelte Bilddatensätze lokal. Das Ergebnis kann sich noch ändern.', 'ExifTool vérifie localement les balises standard, aperçus intégrés et images imbriquées. Le résultat peut encore changer.') : failed ? `${diagnostic?.message ?? t('ExifTool stopped before the full scan finished.', 'ExifTool 在完整扫描结束前停止了。', 'ExifTool stoppte vor Abschluss des Vollscans.', 'ExifTool s’est arrêté avant la fin de l’analyse complète.')} ${t('The browser-only result remains usable.', '浏览器初步结果仍可使用。', 'Das Browser-Ergebnis bleibt nutzbar.', 'Le résultat initial du navigateur reste utilisable.')}` : t(`${report.detectedFieldCount} normalized fields were considered. The image never left this tab.`, `共考虑了 ${report.detectedFieldCount} 个规范化字段，图片从未离开当前标签页。`, `${report.detectedFieldCount.toLocaleString(locale)} normalisierte Felder wurden berücksichtigt. Das Bild hat diesen Tab nie verlassen.`, `${report.detectedFieldCount.toLocaleString(locale)} champs normalisés ont été examinés. L’image n’a jamais quitté cet onglet.`)}</p>{pending && stage && <p className="privacy-scan-stage">{localizePrivacyCleanupStage(stage, locale)}</p>}{diagnostic?.original && <details className="privacy-original-diagnostic"><summary className="disclosure-summary"><span className="disclosure-label">{privacyCleanupText(locale, 'originalDiagnostic')}</span><DisclosureChevron /></summary><p>{diagnostic.original}</p></details>}</div>
