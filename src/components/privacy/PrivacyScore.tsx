...
function conclusion(report: PrivacyReport, locale: Locale): string {
  ...
  if (locale === 'fr') {
    const parts = [report.summary.hasPreciseLocation ? 'une position précise' : report.summary.hasApproximateLocation ? 'un lieu nommé' : '', report.summary.hasCaptureTime ? 'la date de prise de vue' : '', report.summary.hasDeviceIdentifier ? 'un identifiant d’appareil' : '', report.summary.hasIdentityInformation ? 'des informations d’identité' : ''].filter(Boolean);
    if (!parts.length) return 'Aucune métadonnée sensible prise en charge n’a été détectée dans le résultat actuel.';
    const tail = parts.pop();
    return `Cette image contient ${parts.length ? `${parts.join(', ')} et ` : ''}${tail}.`;
  }
...
export function PrivacyScore({ report, pending = false, locale = 'en' }: { report: PrivacyReport; pending?: boolean; locale?: Locale }) {
  ...
  const label = pending ? t('Full scan running', '正在完整扫描', 'Vollscan läuft', 'Analyse complète en cours') : report.completeness === 'embedded' && exif?.status === 'complete' ? t('Full scan complete', '完整扫描完成', 'Vollscan abgeschlossen', 'Analyse complète terminée') : t('Full scan incomplete', '完整扫描未完成', 'Vollscan unvollständig', 'Analyse complète inachevée');
  const level = zh ? ({ Low: '低', Moderate: '中', High: '高', Critical: '严重' }[report.level] ?? report.level) : de ? ({ Low: 'Niedrig', Moderate: 'Mittel', High: 'Hoch', Critical: 'Kritisch' }[report.level] ?? report.level) : fr ? ({ Low: 'Faible', Moderate: 'Modéré', High: 'Élevé', Critical: 'Critique' }[report.level] ?? report.level) : report.level;
  return <section className={`privacy-scoreboard level-${report.level.toLowerCase()}`} aria-labelledby="privacy-score-heading">
    <div className="privacy-score-number"><span id="privacy-score-heading">{privacyCleanupText(locale, 'riskScore')}</span><strong>{report.score}</strong><small>/ 100 · {t('rule-based', '规则评估', 'regelbasiert', 'selon des règles')}</small><small className="privacy-score-direction">{privacyCleanupText(locale, 'lowerIsBetter')}</small></div>
    <div className="privacy-score-reading"><span className="eyebrow">{label}</span><h2>{level}</h2><p>{conclusion(report, locale)}</p>{pending ? <p className="privacy-score-delta">{t('The score can change while ExifTool checks embedded previews and nested records.', 'ExifTool 检查内嵌预览和嵌套记录时，分数仍可能变化。', 'Der Wert kann sich noch ändern, während ExifTool eingebettete Vorschauen und verschachtelte Datensätze prüft.', 'Le score peut encore changer pendant qu’ExifTool vérifie les aperçus et données imbriquées.')}</p> : null}<div className="privacy-progress" role="progressbar" aria-label={`${privacyCleanupText(locale, 'riskScore')} ${report.score}/100 · ${level}. ${privacyCleanupText(locale, 'lowerIsBetter')}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={report.score}><i style={{ width: `${report.score}%` }} /></div></div>
    <dl><div><dt>{t('Risks', '风险', 'Risiken', 'Risques')}</dt><dd>{report.risks.length.toLocaleString(locale)}</dd></div><div><dt>{t('High priority', '高优先级', 'Hohe Priorität', 'Priorité élevée')}</dt><dd>{highCount.toLocaleString(locale)}</dd></div><div><dt>{t('Sensitive fields', '敏感字段', 'Sensible Felder', 'Champs sensibles')}</dt><dd>{report.sensitiveFieldCount.toLocaleString(locale)}</dd></div></dl>
  </section>;
}
