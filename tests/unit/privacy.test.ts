...
describe('image privacy rules', () => {
  ...
  it('does not score prompt, model-setting, or workflow-shaped application fields', () => {
    const value = report([{key:'Prompt',value:'editorial portrait in rain',group:'technical'},{key:'Seed',value:42,group:'technical'},{key:'Workflow',value:{'1':{class_type:'Sampler'}},group:'technical'}]);
    expect(value).toMatchObject({ score: 0, risks: [] });
  });

  it('detects and masks a Windows user path', () => { const item=risk([{key:'SourceFile',value:'C:\\Users\\Jonathan\\Pictures\\draft.png',group:'technical'}], 'local-file-path'); expect(item?.fields[0]?.displayValue).toContain('Jo***'); expect(item?.fields[0]?.rawValue).toContain('Jonathan'); });
  it('detects and masks a macOS user path', () => expect(risk([{key:'Input',value:'/Users/morgan/Models/reference.bin',group:'technical'}], 'local-file-path')?.fields[0]?.displayValue).toContain('/Users/mo***/'));
  it('detects and masks a Linux home path', () => expect(risk([{key:'Input',value:'/home/alex/work/private.png',group:'technical'}], 'local-file-path')?.fields[0]?.displayValue).toContain('/home/al***/'));
  it('detects a private IP without opening it', () => expect(risk([{key:'Endpoint',value:'http://192.168.1.20:8188/view',group:'technical'}], 'internal-network-address')).toMatchObject({score:10,severity:'medium'}));
  it('redacts a credential-bearing URL preview and scores it high', () => { const item=risk([{key:'Callback',value:'https://example.test/job?access_token=secret-123',group:'technical'}], 'internal-network-address'); expect(item).toMatchObject({score:25,severity:'high'}); expect(item?.fields[0]?.displayValue).toContain('[redacted]'); });
});
...
describe('safe privacy report export', () => {
  ...
  it('contains the documented schema 1.3 top-level report fields', () => expect(Object.keys(createSafePrivacyExport(report([])))).toEqual(['version','evidencePolicyVersion','generatedAt','file','completeness','engines','score','level','scoreTimeline','sourceStats','fieldStats','summary','detectedFieldCount','sensitiveFieldCount','risks','warnings','scanWarnings','disclaimer']));
  ...
  it('does not include worker or binary image fields', () => { const exported=createSafePrivacyExport(report([{key:'ApplicationData',value:{'1':{}},group:'technical'}])); const text=JSON.stringify(exported); expect(text).not.toMatch(/blob:|worker|thumbnailBytes|arrayBuffer/i); });
});
