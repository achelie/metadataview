...
function pngChunk(type: string, data=Buffer.alloc(0)): Buffer { const name=Buffer.from(type); const length=Buffer.alloc(4); length.writeUInt32BE(data.length); const crc=Buffer.alloc(4); crc.writeUInt32BE(crc32(Buffer.concat([name,data]))); return Buffer.concat([length,name,data,crc]); }
function textChunk(type: 'tEXt'|'zTXt'|'iTXt', key: string, value: string): Buffer {
  if (type==='tEXt') return pngChunk(type,Buffer.concat([Buffer.from(key,'latin1'),Buffer.from([0]),Buffer.from(value,'latin1')]));
  if (type==='zTXt') return pngChunk(type,Buffer.concat([Buffer.from(key,'latin1'),Buffer.from([0,0]),Buffer.from(deflate(Buffer.from(value)))]));
  return pngChunk(type,Buffer.concat([Buffer.from(key,'latin1'),Buffer.from([0,0,0,0,0]),Buffer.from(value)]));
}
function png(texts: Array<['tEXt'|'zTXt'|'iTXt',string,string]> = []): Buffer {
  const ihdr=Buffer.alloc(13); ihdr.writeUInt32BE(2,0); ihdr.writeUInt32BE(1,4); ihdr.set([8,6,0,0,0],8);
  const scan=Buffer.from([0,239,106,56,255,217,255,67,255]);
  return Buffer.concat([signature,pngChunk('IHDR',ihdr),...texts.map((item)=>textChunk(...item)),pngChunk('IDAT',Buffer.from(deflate(scan))),pngChunk('IEND')]);
}

async function canvasImage(page: Page, mime: 'image/jpeg'|'image/webp'): Promise<Buffer> {
  const values=await page.evaluate(async(type)=>{ const canvas=document.createElement('canvas'); canvas.width=3; canvas.height=2; const context=canvas.getContext('2d')!; context.fillStyle='#ef6a38'; context.fillRect(0,0,3,2); const blob=await new Promise<Blob>((resolve,reject)=>canvas.toBlob((value)=>value?resolve(value):reject(new Error('Canvas encoding failed')),type,.9)); return Array.from(new Uint8Array(await blob.arrayBuffer())); },mime);
...
function withExif(jpeg: Buffer): Buffer {
  ...
  const storeAscii=(value:string)=>{ const bytes=Buffer.from(`${value}\0`,'ascii'); const offset=cursor; bytes.copy(tiff,cursor); cursor+=bytes.length; return {offset,count:bytes.length}; };
  const storeRationals=(values:number[])=>{ const offset=cursor; for(const value of values){ tiff.writeUInt32LE(value,cursor); tiff.writeUInt32LE(1,cursor+4); cursor+=8; } return offset; };
  ...
  const make=storeAscii('Pocket Camera'),model=storeAscii('DeskCam 42'),software=storeAscii('MetadataView Fixture'),artist=storeAscii('Ada Example');
  const date=storeAscii('2024:03:12 14:05:06'),serial=storeAscii('BODY-12345'),lens=storeAscii('35mm Test Lens'); const lat=storeRationals([37,46,30]),lon=storeRationals([122,25,10]);
  tiff.writeUInt16LE(7,ifd0); let e=ifd0+2;
  for(const spec of [[0x010f,2,make.count,make.offset],[0x0110,2,model.count,model.offset],[0x0112,3,1,6],[0x0131,2,software.count,software.offset],[0x013b,2,artist.count,artist.offset],[0x8769,4,1,exifIfd],[0x8825,4,1,gpsIfd]] as const){ entry(e,spec[0],spec[1],spec[2],spec[3]); e+=12; } tiff.writeUInt32LE(0,e);
  tiff.writeUInt16LE(3,exifIfd); e=exifIfd+2; for(const spec of [[0x9003,2,date.count,date.offset],[0xa431,2,serial.count,serial.offset],[0xa434,2,lens.count,lens.offset]] as const){ entry(e,spec[0],spec[1],spec[2],spec[3]); e+=12; } tiff.writeUInt32LE(0,e);
  ...
  const payload=Buffer.concat([Buffer.from('Exif\0\0','binary'),tiff.subarray(0,cursor)]); const length=Buffer.alloc(2); length.writeUInt16BE(payload.length+2);
